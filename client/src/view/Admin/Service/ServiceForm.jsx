...
export default function ServiceForm() {
  ...
  // CRUD
  const { createData, updateData } = useApiHook("/admin/service");

  // Single data (edit)
  const { data: service, loading } = useApiHook(
    id ? `/admin/service/${id}` : null,
  );
  ...
  return (
    <>
      <HeaderSection
        title={`Service ${id ? "Edit" : "Create"}`}
        backLink={"/admin/service"}
      />
          ...
          <div className="w-full flex flex-wrap items-end">
            ...
            <div className="w-full md:w-1/2 p-1">
              <img
                ...
                src={previewImage?.image || imageUrl(service?.image)}
                alt=""
              />
